...
/*!
   ...
   - Core File
 */
(function(f,d,h){var a=f.util;var g={onParse:function(){c.init();e.core.init()},onReady:function(){var i=_WidgetManager._GetAllData().blog;c.blog={title:i.title,homepage:i.canonicalHomepageUrl,id:blogger_blog_id};e.core.fillDetails()}};f.init=g;var c={persistentKeys:["etag","blog","page","post"],ready:false,page:[],post:[],init:function(){c.load();c.getFeedData()},save:function(){if(!d.jStorage.storageAvailable()){return}var i={};d.each(c.persistentKeys,function(j,k){i[k]=c[k]});d.jStorage.set("WB",i)},load:function(){var i=d.jStorage.get("WB");if(!i){return}d.each(i,function(l,j){if(c.persistentKeys.indexOf(l)>-1){c[l]=j}})},getFeedData:function(){var k=function(m,n){var l;d.each(m,function(o,p){l={};l.id=o;l.date=new Date(p.published.$t);l.title=p.title.$t;l.content=p.content.$t;l.link=p.link[p.link.length-1].href;l.labels=[];l.labelsLower=[];if("media$thumbnail" in p){l.thumbnail=p.media$thumbnail.url}if(p.category){d.each(p.category,function(r,q){l.labels.push(q.term);l.labelsLower.push(q.term.toLowerCase())})}c[n].push(l)});l=null};var j={page:false,post:false};var i=function(n,l){var m;d.getJSON("/feeds/"+n+"s/default?orderby=published&v=2&dynamicviews=1&alt=json",function(o){m=o.feed.gd$etag;if(m!=c.etag){k(o.feed.entry,n)}j[n]=true;if(j[l]==true){if(m!=c.etag){c.etag=o.feed.gd$etag;c.save()}c.ready=true}o=null}).error(function(o){a.setDelay(i,500,n,l)})};i("post","page");i("page","post")}};f.data=c;var b={};f.elem=b;var e={core:{init_done:false,init:function(){d('<div id="WB"><ul><li class="appmenu"><a class="blogger" href="http://www.blogger.com/" title="Go to Blogger.com"></a><a class="title"></a><div class="menu"><hr/></div></li><li class="search"><form action="/search"><input name="q" type="text" placeholder="Search "><input value type="submit"></form></li></ul></div>').insertBefore(d("#navbar"));b.main=d("#WB");b.appmenu={main:d("li.appmenu",b.main)};b.appmenu.blogger=d("a.blogger",b.appmenu.main);b.appmenu.title=d("a.title",b.appmenu.main);b.search={main:d("li.search",b.main)};b.search.form=d("form",b.search.main);b.search.input=d("input[type=text]",b.search.form);b.search.submit=d("input[type=submit]",b.search.form);e.core.init_done=true;e.core.fillDetails()},filledDetails:false,fillDetails:function(){if(e.core.filledDetails||(!("blog" in c))){return}b.appmenu.title.html(c.blog.title).attr("href",c.blog.homepage);b.search.input.attr("placeholder","Search "+c.blog.title);if(document.referrer.length&&a.parseURL().host==a.parseURL(document.referrer).host){b.main.show()}else{b.main.fadeIn()}e.core.filledDetails=true}}};f.UI=e;g.onParse();d(document).ready(g.onReady)}(window.WB=window.WB||{},jQuery));

/*!
   WookBar for Blogger
   - Search
 */
(function(h,e,a){var b=h.UI,f=h.data,c=h.elem.search,g=h.util;var j=function(){i.attachAll()};var d={input:{focus:function(){c.input.focus()},blur:function(){c.input.blur()}}};var i={attachAll:function(){c.form.hover(i.form.hoverIn,i.form.hoverOut);c.input.on("focus",i.input.focus).on("blur",i.input.blur)},form:{hoverIn:function(k){g.clearDelay(d.input.blur);d.input.focus()},hoverOut:function(k){if(c.input.is(":focus")){g.setDelay(d.input.blur,3000)}else{d.input.blur()}}},input:{focus:function(k){c.form.addClass("focus")},blur:function(k){c.form.removeClass("focus")}},submit:{submit:function(k){k.preventDefault()}}};j()}(window.WB=window.WB||{},jQuery));
