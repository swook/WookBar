...
(function(e,c,g){var f={};function a(k){if(!k){k=document.URL}if(k in f){return f[k]}var j=(k==document.URL)?document.location:c('<a href="'+k+'"/>')[0],i={protocol:j.protocol.slice(0,j.protocol.length-1),host:j.hostname,port:j.port,path:j.pathname,query:j.search,params:j.search.length?(function(){var m=[],l={};c.each(j.search.slice(1).split("&"),function(n,o){if(o.length){m=o.split("=");if(m[1].length){l[m[0]]=m[1]}}});return l}()):"",file:j.pathname.match(/\/([^\/?&]*)[^\/]*$/)[1],hash:j.hash.length?j.hash.slice(1):"",relative:j.href.match(/:\/\/[^\/]+(.*)$/)[1],full:j.href.match(/([^#]*)/)[1],full_hash:j.href};if(k==j.href){f[k]=i}if(j instanceof jQuery){j.remove()}return i}var h=[];function b(j,l){var i,m=murmurhash3_32_gc(this.toString()+j.toString());c.each(h,function(n,o){if(o.func==m){i=o;return false}});var k=setTimeout(j,l,[].splice.call(arguments,2));if(i!=g){clearTimeout(i.timeout);i.timeout=k}else{h.push({func:m,timeout:k})}}function d(i){var j=murmurhash3_32_gc(this.toString()+i.toString());c.each(h,function(k,l){if(l.func==j){clearTimeout(l.timeout);return false}})}e.util={parseURL:a,setDelay:b,clearDelay:d}}(window.WB=window.WB||{},jQuery));

/*! WookBar by Seon-Wook Park (www.swook.net) | CC BY-NC-SA | core.js */
(function(g,d,a){var f=g.util;var i={onParse:function(){e.init();b.core.init()},onReady:function(){var j=_WidgetManager._GetAllData().blog;e.blog={title:j.title,homepage:j.canonicalHomepageUrl,id:blogger_blog_id};b.core.fillDetails();h.attachAll()}};g.init=i;var e={persistentKeys:["etag","blog","page","post"],ready:false,page:[],post:[],init:function(){e.load();e.getFeedData()},save:function(){if(!d.jStorage.storageAvailable()){return}var j={};d.each(e.persistentKeys,function(k,l){j[l]=e[l]});d.jStorage.set("WB",j)},load:function(){var j=d.jStorage.get("WB");if(!j){return}d.each(j,function(m,l){if(e.persistentKeys.indexOf(m)>-1){e[m]=l}})},getFeedData:function(){var l=function(n,o){var m;d.each(n,function(p,q){m={};m.id=p;m.date=new Date(q.published.$t);m.title=q.title.$t;m.content=q.content.$t;m.link=q.link[q.link.length-1].href;m.labels=[];m.labelsLower=[];if("media$thumbnail" in q){m.thumbnail=q.media$thumbnail.url}if(q.category){d.each(q.category,function(s,r){m.labels.push(r.term);m.labelsLower.push(r.term.toLowerCase())})}e[o].push(m)});m=null};var k={page:false,post:false};var j=function(o,m){var n;d.getJSON("/feeds/"+o+"s/default?orderby=published&v=2&dynamicviews=1&alt=json",function(p){n=p.feed.gd$etag;if(n!=e.etag){l(p.feed.entry,o)}k[o]=true;if(k[m]==true){if(n!=e.etag){e.etag=p.feed.gd$etag;e.save()}e.ready=true}p=null}).error(function(p){f.setDelay(j,500,o,m)})};j("post","page");j("page","post")}};g.data=e;var c={};g.elem=c;var b={core:{init_done:false,init:function(){d('<div id="WB"><ul><li class="appmenu"><div class="bar-bg"><div class="bg"/><div class="onhover"/></div><a class="blogger" href="http://www.blogger.com/" title="Go to Blogger.com"></a><a class="title"></a><div class="menu"><hr/>Lorem ipsum dolor sit amet, consectetur adipiscing elit.</div></li><li class="search"><form action="/search"><input name="q" type="text" placeholder="Search "><input value type="submit"></form><div class="results"><div class="result">Lorem ipsum dolor sit amet, consectetur adipiscing elit.</div></div></li></ul></div>').insertBefore(d("#navbar"));c.main=d("#WB");c.appmenu={main:d("li.appmenu",c.main)};c.appmenu.title=d("a.title",c.appmenu.main);c.search={main:d("li.search",c.main)};c.search.form=d("form",c.search.main);c.search.input=d("input[type=text]",c.search.form);b.core.init_done=true;b.core.fillDetails()},filledDetails:false,fillDetails:function(){if(b.core.filledDetails||(!("blog" in e))){return}c.appmenu.title.html(e.blog.title).attr("href",e.blog.homepage);c.search.input.attr("placeholder","Search "+e.blog.title);b.core.reveal();b.core.filledDetails=true},reveal:function(){if(document.referrer.length&&f.parseURL().host==f.parseURL(document.referrer).host){c.main.show()}else{c.main.fadeIn()}}}};g.UI=b;var h={attachAll:function(){c.main.click(h.main.click)},main:{click:function(j){j.stopPropagation()}}};i.onParse();d(document).ready(i.onReady)}(window.WB=window.WB||{},jQuery));

/*! WookBar by Seon-Wook Park (www.swook.net) | CC BY-NC-SA | appmenu.js */
(function(h,e,a){var b=h.UI,f=h.data,c=h.elem.appmenu,g=h.util;c.blogger=e("a.blogger",c.main);var j=function(){i.attachAll()};var d={main:{focus:function(){c.main.addClass("hover");g.clearDelay(d.main.blur)},blur:function(){c.main.removeClass("hover");g.clearDelay(d.main.blur)}}};var i={attachAll:function(){c.main.hover(i.main.hoverIn,i.main.hoverOut);e("html").click(i.body.click)},body:{click:function(k){d.main.blur()}},main:{hoverIn:function(k){d.main.focus()},hoverOut:function(k){g.setDelay(d.main.blur,1000)}}};j()}(window.WB=window.WB||{},jQuery));

/*! WookBar by Seon-Wook Park (www.swook.net) | CC BY-NC-SA | search.js */
(function(h,e,a){var b=h.UI,f=h.data,c=h.elem.search,g=h.util;c.submit=e("input[type=submit]",c.form);c.results=e("div.results",c.main);var j=function(){i.attachAll()};var d={input:{focus:function(){c.input.focus()},blur:function(){c.input.blur()}}};var i={attachAll:function(){c.main.hover(i.main.hoverIn,i.main.hoverOut);c.input.on("focus",i.input.focus).on("blur",i.input.blur);e(window).resize(i.window.resize);i.window.resize()},window:{resize:function(k){c.results.css("max-height",window.innerHeight-26)}},main:{hoverIn:function(k){g.clearDelay(d.input.blur);d.input.focus()},hoverOut:function(k){if(c.input.is(":focus")){g.setDelay(d.input.blur,3000)}else{d.input.blur()}}},input:{focus:function(k){c.main.addClass("focus")},blur:function(k){c.main.removeClass("focus")}},submit:{submit:function(k){k.preventDefault()}}};j()}(window.WB=window.WB||{},jQuery));

/*! WookBar by Seon-Wook Park (www.swook.net) | CC BY-NC-SA | social.js */
(function(f,d,h){var c=f.data,a=f.util;function g(){}var b={main:d("li.social",f.elem.main),};b.menu=d("div.menu",b.main);var e={}}(window.WB=window.WB||{},jQuery));
...
(function(f,d,g){var e=f.UI,c=f.data,b=f.elem,a=f.util;d.Ajaxify.parseUri=a.parseURL}(window.WB=window.WB||{},jQuery));
